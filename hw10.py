...
def conf_matrix(y_pred, y_true, num_class):
    ...
    confmatrix = np.zeros((num_class, num_class))

    for i in range(num_class):
        for j in range(num_class):
            confmatrix[i, j] = np.sum((y_true == classes[i]) & (y_pred == classes[j]))
...
def get_model(name, params):
    ...
    params : list of parameters cooresponding to given model
...
def get_model_results(model_name, params, train_data, train_labels, test_data, test_labels, num_class):
    ...
    auc_scor = metrics.roc_auc_score(test_labels, prob, multi_class='ovr')
